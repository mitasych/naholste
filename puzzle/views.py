...
from collage.common.helpers import CropImg, CookiePuzzleFiles, CookiePuzzleOption
from collage.puzzle.models import PuzzleFiles, PuzzleOption
from collage.crop.models import Frames, Packaging
from collage.puzzle.forms import UploadForm, OptionForm
...
def upload(request):
	...
	if (user.is_authenticated() and user.is_active):
		if PuzzleFiles.objects.filter(user=user.id).exists():
			thumbs = PuzzleFiles.objects.filter(user=user.id)
	else:
...
def delete_file(request, file_id=''):
	...
	if request.is_ajax():
		...
		###
		if (user.is_authenticated() and user.is_active):
			if PuzzleFiles.objects.filter(user=user.id, char_id=file_id).exists():
				row = PuzzleFiles.objects.get(user=user.id, char_id=file_id)
				###
				DataImg().del_auth(file_id, row.pth)
				###
				PuzzleFiles.objects.filter(user=user.id, char_id=file_id).delete()
				###
...
def option(request, file_id=''):
	...
	if (user.is_authenticated() and user.is_active):
		if PuzzleFiles.objects.filter(user=user.id, char_id=file_id).exists():
			...
			ins = False
			###
			if PuzzleOption.objects.filter(file=row).exists():
				opt = PuzzleOption.objects.get(file=row)
				###
				ins = True
			else:
		...
		if file_id in data:
			no_redirect = True
			###
			row = PuzzleFiles().getRow(file_id)
			###
			not_found = True
			###
			for i in opts:
